...
class CapsuleFC(nn.Module):
    ...
    def forward(self, input, num_iter, next_capsule_value=None):
        ...
        if next_capsule_value is None:
            ...
            if self.matrix_pose:
                # print(query_key.shape, _input.shape, w.shape) # torch.Size([3360, 4]) torch.Size([1, 39200, 4, 4]) torch.Size([3360, 4, 4, 4])
...
class CapsuleCONV(nn.Module):
    ...
    def __init__(self, in_n_capsules, in_d_capsules, out_n_capsules, out_d_capsules,
                 kernel_size, stride, matrix_pose, dp, padding=(1, 0), coordinate_add=False):
        ...
        if matrix_pose:
            ...
            self.weight_init_const = np.sqrt(
                out_n_capsules / (self.sqrt_d * in_n_capsules * kernel_size[0] * kernel_size[1]))
            self.w = nn.Parameter(self.weight_init_const * torch.randn(kernel_size[0], kernel_size[1],
                                                                       in_n_capsules, self.sqrt_d, self.sqrt_d,
                                                                       out_n_capsules))
        else:
            self.weight_init_const = np.sqrt(
                out_n_capsules / (in_d_capsules * in_n_capsules * kernel_size[0] * kernel_size[1]))
            self.w = nn.Parameter(self.weight_init_const * torch.randn(kernel_size[0], kernel_size[1],
    ...
    def input_expansion(self, input):
        ...
        h_out = int((h - self.kernel_size[0]) / self.stride + 1)
        w_out = int((w - self.kernel_size[1]) / self.stride + 1)

        # this may be slow if the image size is large
        # TODO: kind of stupid implementation :'(
        # torch.stack()
        # 同样是对tensors沿指定维度拼接，但返回的Tensor会多一维
        inputs = torch.stack([input[:, :, self.stride * i:self.stride * i + self.kernel_size[0],
                              self.stride * j:self.stride * j + self.kernel_size[1], :] for i in range(h_out) for j in
                              range(w_out)],
                             dim=-1)  # b,n, kernel_size, kernel_size, h_out*w_out*d
        inputs = inputs.view(b, n, self.kernel_size[0], self.kernel_size[1], h_out, w_out, d)
        return inputs
    ...
    def forward(self, input, num_iter, next_capsule_value=None):
        ...
        # print(inputs.shape) # torch.Size([8, 32, 3, 3, 37, 5, 8])
        if self.matrix_pose:
            w = self.w  # klnxdm
        ...
        if next_capsule_value is None:
            query_key = torch.zeros(self.in_n_capsules, self.kernel_size[0], self.kernel_size[1],
        ...
        next_capsule_value = self.drop(next_capsule_value)
        # print(next_capsule_value.shape) # torch.Size([8, 32, 37, 5, 4, 4])
        if not next_capsule_value.shape[-1] == 1:
            if self.matrix_pose:
                next_capsule_value = next_capsule_value.view(next_capsule_value.shape[0],
                ...
                next_capsule_value = self.nonlinear_act(next_capsule_value)
        # print(next_capsule_value.shape)
        return next_capsule_value
